...
TEST(DENSE, TEST_1) {
    ...
    std::vector<std::vector<double>> r{{-8.124038404635959, -9.601136296387955, -11.07823418813995}, {0, 9.045340337332926,1.809068067466585}, {0,0,0}};
    ...
    for(std::size_t i = 0; i < Q.get_height() * Q.get_width(); i++)
        ASSERT_NEAR(QR.first.get_values()[0], Q.get_values()[0], 1e-12);
    for(std::size_t i = 0; i < R.get_height() * R.get_width(); i++)
        ASSERT_NEAR(QR.second.get_values()[0], R.get_values()[0], 1e-12);
}
